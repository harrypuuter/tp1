...
C...Read generation scale for all FS particles (as comment in event file)
      IF(ickkw.eq.1)THEN
        READ(LNHIN,*,END=900,ERR=130) CDUM,(PTCLUS(I),I=1,NEX)
 130    CONTINUE
      ENDIF
...
c           Trace mothers, if non-radiating => daughter is decay - remove
 ...
 105        ISTHEP(ihep)=2
      ...
      CALL ALPSOR(ptsort,nljets,KP,1)

c     Print out HEPEVT record
      if(idbg.eq.1)then
         write(*,*) 'After removing non-partons'
         CALL PYLIST(5)
         write(*,*) 'ptsort: ',(ptsort(j),j=1,i)
      endif
...
C     Veto events where first shower emission has kt > YCUT
     ...
     $          qcut,ptsort(1),vint(357),vint(358),vint(360)
        ...
        IF(NLJETS.GT.0.AND.PTSORT(1)**2.LT.YCUT) THEN
          if(idbg.eq.1)
...
c        PRINT *,'Y,VINT:',SQRT(Y(NLJETS+1)),SQRT(VINT(390))
     ...
     $       'Failed due to ',max(VINT(357),VINT(358)),' > ',SQRT(YCUT)
          GOTO 999
        ENDIF
c        PRINT *,NLJETS,IEXC,SQRT(VINT(390)),PTSORT(1),SQRT(YCUT)
c     Highest multiplicity case
        IF(IEXC.EQ.0.AND.NLJETS.GT.0.AND.
     $       ((mektsc.eq.1.and.MAX(VINT(357),VINT(358)).GT.PTSORT(1))
     $       .OR.
     $       (mektsc.eq.2.and.MAX(VINT(360),VINT(358)).GT.PTSORT(1))))
     $       THEN
c     $     VINT(390).GT.PTSORT(1)**2)THEN
          if(idbg.eq.1)
     $       WRITE(LNHOUT,*),
     $       'Failed due to ',max(VINT(357),VINT(358)),' > ',PTSORT(1)
          GOTO 999
        ENDIF
c     
