...
C     Only run if IMODE is 0
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d d~ > u d~ e- ve~
      PD(IPROC)=PD(IPROC-1) + D1*DB2
      IPROC=IPROC+1  ! s s~ > u d~ e- ve~
      PD(IPROC)=PD(IPROC-1) + S1*SB2
      IPROC=IPROC+1  ! d d~ > u d~ mu- vm~
      PD(IPROC)=PD(IPROC-1) + D1*DB2
      IPROC=IPROC+1  ! s s~ > u d~ mu- vm~
      PD(IPROC)=PD(IPROC-1) + S1*SB2
      IPROC=IPROC+1  ! d d~ > c s~ e- ve~
      PD(IPROC)=PD(IPROC-1) + D1*DB2
      IPROC=IPROC+1  ! s s~ > c s~ e- ve~
      PD(IPROC)=PD(IPROC-1) + S1*SB2
      IPROC=IPROC+1  ! d d~ > c s~ mu- vm~
      PD(IPROC)=PD(IPROC-1) + D1*DB2
      IPROC=IPROC+1  ! s s~ > c s~ mu- vm~
      PD(IPROC)=PD(IPROC-1) + S1*SB2
      IF (IMODE.EQ.4)THEN
        DSIG2 = PD(IPROC)
        RETURN
