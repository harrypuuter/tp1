...
C     
C     Process: u u~ > w+ w- WEIGHTED=4
C     *   Decay: w+ > u d~ WEIGHTED=2
C     *   Decay: w- > e- ve~ WEIGHTED=2
C     Process: c c~ > w+ w- WEIGHTED=4
C     *   Decay: w+ > u d~ WEIGHTED=2
C     *   Decay: w- > e- ve~ WEIGHTED=2
C     Process: u u~ > w+ w- WEIGHTED=4
C     *   Decay: w+ > u d~ WEIGHTED=2
...
C     Only run if IMODE is 0
      ...
      IF (ABS(LPP(IB(2))).GE.1) THEN
        LP=SIGN(1,LPP(IB(2)))
        CB2=PDG2PDF(ABS(LPP(IB(2))),-4*LP,XBK(IB(2)),DSQRT(Q2FACT(2)))
        UB2=PDG2PDF(ABS(LPP(IB(2))),-2*LP,XBK(IB(2)),DSQRT(Q2FACT(2)))
      ENDIF
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u u~ > u d~ e- ve~
      PD(IPROC)=PD(IPROC-1) + U1*UB2
      IPROC=IPROC+1  ! c c~ > u d~ e- ve~
      PD(IPROC)=PD(IPROC-1) + C1*CB2
      IPROC=IPROC+1  ! u u~ > u d~ mu- vm~
      PD(IPROC)=PD(IPROC-1) + U1*UB2
      IPROC=IPROC+1  ! c c~ > u d~ mu- vm~
      PD(IPROC)=PD(IPROC-1) + C1*CB2
      IPROC=IPROC+1  ! u u~ > c s~ e- ve~
      PD(IPROC)=PD(IPROC-1) + U1*UB2
      IPROC=IPROC+1  ! c c~ > c s~ e- ve~
      PD(IPROC)=PD(IPROC-1) + C1*CB2
      IPROC=IPROC+1  ! u u~ > c s~ mu- vm~
      PD(IPROC)=PD(IPROC-1) + U1*UB2
      IPROC=IPROC+1  ! c c~ > c s~ mu- vm~
      PD(IPROC)=PD(IPROC-1) + C1*CB2
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(IPROC)
        RETURN
